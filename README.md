...
Скрипт синхронизирует файлы через `rsync` и запускает деплой по SSH в tmux-сессии — сборка образа (~60 мин) переживёт обрыв соединения. Если связь пропала, переподключитесь и выполните `tmux attach -t deploy`.
