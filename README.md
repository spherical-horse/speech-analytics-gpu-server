...
```
[локальная машина]          [GHCR]              [сервер]
  bash build.sh    →→→→→→  образ  →→→→→→→→   deploy.sh
  (сборка + push)                           (pull + запуск)
```

1. **Один раз**: подготовьте образ локально — `bash build.sh`
2. **Деплой**: `bash deploy-remote.sh user@server` — синхронизирует файлы и запускает сервер

При обновлении: пересоберите образ с новым тегом, задеплойте снова.

---

## Деплой на чистый сервер

Скрипт `deploy.sh` автоматически устанавливает все зависимости и запускает сервис.
...
7. Скачивает образ из GHCR (`docker compose pull`)
...
Образ собирается локально и публикуется в GitHub Container Registry. На сервере он только скачивается.

### Подготовка

1. Скопируйте `.env.example` в `.env` и заполните:
   - `HF_TOKEN` — для скачивания ML-моделей при сборке
   - `GHCR_IMAGE` — полное имя образа, например `ghcr.io/username/speech-analytics-gpu-server`
   - `GHCR_USER` — ваш GitHub-логин
   - `GHCR_TOKEN` — Personal Access Token со scope `write:packages` ([создать](https://github.com/settings/tokens))

2. Сделайте пакет публичным в настройках репозитория на GitHub (или оставьте приватным — тогда на сервере тоже нужен токен с `read:packages`).

### Сборка и публикация

```bash
# Собрать и опубликовать с тегом latest
bash build.sh

# Собрать с конкретным тегом
bash build.sh --tag v1.2.0
```
